...
st.checkbox(f"Clean data for {file.name}")
...
if st.button(f"Convert {file.name}"):
    ...
    df.to_excel(buffer, index=False, engine='openpyxl')  # Engine specify karna zaroori hai
    file_name = file.name.replace(file_ext, ".xlsx")
    mime_type = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
    buffer.seek(0)  # Buffer ko start position pe set karein
